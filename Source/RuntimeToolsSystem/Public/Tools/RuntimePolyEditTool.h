...
public:
    ...
    // Multiple base classes have a UWorld* TargetWorld, causes scoping problems...so we add a third that we can rely
    // on...
    UPROPERTY()
    TObjectPtr<UWorld> LocalTargetWorld = nullptr;

    // Override this to work around a bug in PolyEditActivityUtil::CreatePolyEditPreviewMesh() that calls GetWorld
    // instead of GetTargetWorld()
