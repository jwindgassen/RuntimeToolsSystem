 ...
 */
UENUM()
enum class EToolActorInteractionMode : uint8 { NoInteraction, RightMouseCameraControl, AltCameraControl };
 ...
 * than using the ITF. (*However*, the alt-controls are not fully implemented, ie it is not Maya-style
 * alt+3-mouse-button controls)
 */
UCLASS()
...
public:
    ...
    // set in PossessedBy(), we keep track of this so that the ToolsFramework can access it to figure out mouse cursor
    // rays
    UPROPERTY()
    ...
    EToolActorInteractionMode GetCurrentInteractionMode() {
