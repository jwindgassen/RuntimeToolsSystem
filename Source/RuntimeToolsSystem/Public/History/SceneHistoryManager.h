...
/**
 * FChangeHistoryRecord is a (UObject, FCommandChange, Description) tuple, as a UStruct so that the UObject can be kept
 * alive for GC purposes
 */
USTRUCT()
...
struct FChangeHistoryTransaction {
    ...
    /** @return true if all contained FCommandChange's have Expired. In this case, the entire Transaction will have no
     * effect, and should be skipped in Undo/Redo. */
    bool HasExpired() const;
};


/**
 * USceneHistoryManager provides a simple undo/redo history stack. In this context a "Transaction" is not a UObject
 * property transaction, but simply a list of FCommandChange objects. Transaction terminology is used here to be make
 * the relationship clear with various InteractiveToolsFramework APIs.
 *
 * Note that FCommandChange's are paired with UObjects, similar to the Editor Transaction system. This is necessary
 * because the Change implementations generally do not store the target UObject (because that's how the Editor works).
...
public:
    ...
    bool IsBuildingTransaction() const {
        return BeginTransactionDepth > 0;
    }
...
protected:
    ...
    // remove any elements of Transactions list beyond CurrentIndex (called if we are in Undo state and a new
