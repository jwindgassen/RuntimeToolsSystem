...
public:
    ...
    virtual void AppendChange(UObject* TargetObject, TUniquePtr<FToolCommandChange> Change, const FText& Description)
        override {
...
void UToolsSubsystem::InitializeToolsContext(UWorld* TargetWorldIn) {
    ...
    SelectionInteraction->Initialize([this]() { return HaveActiveTool() == false; });
    ...
    TransformInteraction->Initialize([this]() { return HaveActiveTool() == false; });
...
public:
    ...
    FRuntimeToolsFrameworkRenderImpl(
        UToolsContextRenderComponent* RenderComponentIn, const FSceneView* ViewIn, FViewCameraState CameraState
    ) :
...
void UToolsSubsystem::Tick(float DeltaTime) {
    ...
    InputState.SetModifierKeyStates(
        ModifierState.IsLeftShiftDown(), ModifierState.IsAltDown(), ModifierState.IsControlDown(),
    ...
    if (ViewportClient) {
        ...
        FSceneViewFamilyContext ViewFamily(
            FSceneViewFamily::ConstructionValues(ViewportClient->Viewport, TargetWorld->Scene, *ShowFlags)
        ...
        FSceneView* SceneView = LocalPlayer->CalcSceneView(
            &ViewFamily, /*out*/ ViewLocation, /*out*/ ViewRotation, LocalPlayer->ViewportClient->Viewport
        );
        ...
        if (!ViewportClient->IsOrtho()) {
            ...
            Direction = InvViewMatrix
                            .TransformVector(FVector(InvProjMatrix.TransformFVector4(FVector4(
                                ScreenX * GNearClippingPlane, ScreenY * GNearClippingPlane, 0.0f, GNearClippingPlane
                            ))))
                            .GetSafeNormal();
        } else {
            Origin =
                InvViewMatrix.TransformFVector4(InvProjMatrix.TransformFVector4(FVector4(ScreenX, ScreenY, 0.5f, 1.0f))
                );
            ...
            if (Name == TEXT("CachedProperties")) {
                ...
                if (const FObjectProperty* ObjectProperty = CastField<FObjectProperty>(Property)) {
                    UObject* TargetObject =
                        ObjectProperty->GetObjectPropertyValue(Property->ContainerPtrToValuePtr<UObject*>(CDO));
